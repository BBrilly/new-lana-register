...
Deno.serve(async (req) => {
  ...
  console.log('🚀 Kind 87005 - New LanaCoins Registration Publisher started');
  ...
  try {
    ...
    // 3. Fetch unpublished registered_lana_events
    console.log('📋 Fetching unpublished registered_lana_events...');
    const { data: unpublishedEvents, error: eventsError } = await supabase
      .from('registered_lana_events')
      .select('id, wallet_id, amount, split, block_id, transaction_id, notes, detected_at')
      .eq('nostr_87005_published', false)
    ...
    if (!unpublishedEvents || unpublishedEvents.length === 0) {
      console.log('✅ No unpublished events to process');
      return new Response(
        JSON.stringify({
          success: true,
          message: 'No unpublished events to process',
          processed: 0,
        }),
      ...
      try {
        ...
        const eventTemplate = {
          ...
          tags: [
            ...
            ['Agreement', 'Knights Agreement'],
            ['Split_ID', `split_${event.split}`],
            ['Registrar', sysParams.pubkey || ''],
            ['block_id', event.block_id?.toString() || ''],
            ['transaction_id', event.transaction_id || ''],
          ],
          content: 'New LanaCoins registration request',
        };
        ...
        if (publishSuccess) {
          ...
          const { error: updateError } = await supabase
            .from('registered_lana_events')
            .update({
              nostr_87005_published: true,
              nostr_87005_event_id: eventId,
              nostr_87005_published_at: new Date().toISOString(),
            })
            .eq('id', event.id);
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Processed ${unpublishedEvents.length} events`,
